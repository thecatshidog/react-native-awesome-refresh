...
import com.reactnativeawesomerefresh.awesomerefreshlayout.Events;
import com.reactnativeawesomerefresh.awesomerefreshlayout.ReactSmartRefreshLayout;
import com.reactnativeawesomerefresh.header.AnyHeader;
...
public class AwesomeRefreshViewManager extends ViewGroupManager<ReactSmartRefreshLayout>{
    //返回给rn的组件名
    protected static final String REACT_CLASS="AwesomeRefreshView";
    ...
    public void receiveCommand(ReactSmartRefreshLayout root, int commandId, @Nullable ReadableArray args) {
        switch (commandId){
            case COMMAND_FINISH_REFRESH_ID:
                ...
                if(delayed>=0){
                    root.finishRefresh(delayed,success, Boolean.TRUE);
                }else{
    ...
    protected void addEventEmitters(ThemedReactContext reactContext,final ReactSmartRefreshLayout view) {
        ...
        view.setOnMultiListener(new SimpleMultiListener() {
            ...
            public void onHeaderMoving(RefreshHeader header, boolean isDragging, float percent, int offset, int headerHeight, int maxDragHeight) {
                ...
                writableMap.putBoolean("isDragging", isDragging);
                ...
                writableMap.putDouble("maxDragHeight", SmartUtil.px2dp(maxDragHeight));
